...
{  
    ...
    if (!charityCell.displayImageView.image) {
        charityCell.displayImageView.image = [self charityImageURLSForSpecifcCharity:indexPath.row];
    }
    ...
    if ([[resultOfCharitableConversionsArray objectAtIndex:indexPath.row] integerValue] == 1) {
        charityDescription = [self charityDescriptionSingular:indexPath.row];
    } else {
        charityDescription = [self charityDescriptionPlural:indexPath.row];
    }
...
{
    charityImagesArray = [NSArray array];
    charityImagesArray = @[@[
